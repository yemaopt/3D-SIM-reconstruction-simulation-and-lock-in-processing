...
function [ I,X,Y ] = FocusedPatternXY_CZT_RealUnit(Intensity,PhasePlate,Polarization,z,CalculationRange2,CalculationStep2)

lambda=0.488; %um
z2=z; %um
SamplingRange2=80;%um
SamplingStep2=1/80; %um
...
switch PhasePlate.Name
    ...
    case 'CH-STED'
        rc=PhasePlate.InnerRadius;
        thetac=asin(rc/f/n);
        Phase=exp(1i.*phi3.*double(theta3<=thetac)).*exp(1i.*(phi3+pi).*double(theta3>thetac));
        Phase=Phase.*Pupil;
    case 'CH-STED Inner'
        rc=PhasePlate.InnerRadius;
        thetac=asin(rc/f/n);
        Phase=exp(1i.*phi3.*double(theta3<=thetac));
        Phase=Phase.*double(theta3<=thetac);
    case 'CH-STED Outer'
        rc=PhasePlate.InnerRadius;
        thetac=asin(rc/f/n);
        Phase=exp(1i.*(phi3+pi).*double(theta3>thetac));
        Phase=Phase.*double(theta3>thetac).*Pupil;
    case 'Test'
        rc=PhasePlate.InnerRadius;
        thetac=asin(rc/f/n);
        Phase1=exp(1i.*phi3.*double(theta3<=thetac));
        Phase1=Phase1.*double(theta3<=thetac);
        Phase2=exp(1i.*(phi3+pi).*double(theta3>thetac));
        Phase2=Phase2.*double(theta3>thetac).*Pupil;
        Phase=Phase1+Phase2;
    otherwise
